# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project

Post-processing toolkit for [GENE](http://genecode.org) gyrokinetic plasma simulations, including GENE-3D. Reads Fortran-binary, HDF5 and ADIOS2 output and provides diagnostics and visualization.

## Commands

```bash
# Install (editable)
pip install -e .
pip install -e ".[dev]"       # with pytest

# Run tests (suite lives under tests/)
pytest tests/ -v
pytest tests/io/test_params.py -v -k "test_defaults_applied"   # single test
pytest tests/diagnostics -v                                    # diagnostics only

# Coverage
pytest tests/ -v --cov=genetools --cov-report=term-missing
```

Note: tests are under `tests/` (matching `testpaths = ["tests"]` in `pyproject.toml`). A `genetools` console-script CLI is installed by the editable install.

```bash
pytest tests/ -k gene3d -v     # GENE-3D only
```

`tests/gene_fixture.py` builds synthetic **regular-GENE** runs — `make_fluxtube_run` (with a real
`tracer_efit` geometry file) and `make_xglobal_run`, plus per-file writers `write_srcmom` and
`write_vsp`. No x-global run with output exists in the repo, so that column of
the diagnostics matrix was verified by reading code until this existed — and two `shearingrate` bugs
(the ExB sign and the whole-array Jacobian normalisation) had been hidden precisely because the
x-global *unit* fixtures used a uniform Jacobian and `C_xy = 1`. Extend it rather than adding more
unit-level stand-ins.

GENE-3D has no reference output in the repo; `tests/gene3d_fixture.py` builds
synthetic run directories reproducing what `gene3d-dev/src` writes (futils axis
order and precision, snapshot groups, the deliberately wrong `n_moms`).
`make_gene3d_run(..., physical=True)` derives the fluxes from the fields exactly
as `diag_3d.F90` does, which is what makes the flux reconstruction checkable
rather than merely plausible.

## Architecture

### High-level layer (the recommended entry point)
- **`run.py`** — `Run` facade: `Run(path, ext=None)` auto-wires `set_runs` → `Params` → `Geometry`/`Coordinates` and lazily builds multi-segment readers. Every diagnostic is a lazy attribute (`run.nrg`, `run.spectra`, `run.profiles`, `run.fluxes2d`, `run.shearing`, `run.contours`, `run.growthrate`, `run.amplitude`, `run.profile_diag`, `run.gam`, `run.chi`, `run.omega`, `run.geometry_plots`, `run.srcmom`, `run.vsp`) or a callable where it needs arguments (`run.ballooning(ky=...)`, `run.timetraces(...)`, `run.planes(...)`, `run.vis3d(...)`, `run.frequency(...)`). Each accessor just constructs the one class for that diagnostic — the class handles the geometry, so `run.spectra.plot()` means the same thing whatever the run is. `run.geometry_kind` / `run.is_3d` report which geometry. Validates grid consistency across continuation segments (warns on mismatch; assumes same grid). Every diagnostic offers `.data` (xarray), `.plot(t=(start,stop))`, `.save()`.

  The one exception is `run.nrg`, which still goes through a thin `_BoundNrg` wrapper: `NrgReader` is a folder-based ASCII reader with no geometry branching, so it was left as-is.
- **`_xr.py`** — small explicit helpers for building labelled `xarray.Dataset`s (`stacked_vars` for species-dim stacking, `attach_coords` for length-matched coord attachment, `make_dataset`, `unit_attrs`, `split_species`). Each diagnostic owns its xarray construction: `NrgReader.dataset(params)` and `Spectra`/`SpectraGlobal`/`Profiles`/`Fluxes2D`/`ShearingRate.dataset(coords, params, species, …)` build their Datasets with explicit dims. The compute stays numpy; xarray is the return layer.
- **`cli.py`** — flag-style CLI (`genetools /run --spectra --t 500 2000 [--save fig.png] [--no-show]`), one flag per diagnostic. Entry point: `genetools = genetools.cli:main`.

The repo root **is** the `genetools` package (so `run.py` → `genetools.run`, etc.). `xarray` is a hard dependency; the data layer returns `xarray.Dataset`s.

Two subpackages under the root `genetools` package:

### `io/` — Data loading
- **`params.py`** — `Params` class: parses Fortran-90 namelist parameter files, applies physics defaults (`_DEFAULTS`), computes derived units. Supports multi-segment runs. `_normalise()` reconciles keys the two codes write in different namelists (GENE puts `x_local`/`y_local` in `&general` and `write_h5` in `&in_out`; GENE-3D hard-codes all three into `&info`) and records `info['geometry_kind']` / `info['is_3d']`. `Params.geometry_kind()` / `Params.is_3d()` expose it.
- **`data.py`** — `_BaseReader` ABC with `BinaryReader` (Fortran unformatted), `H5Reader` (GENE `write_h5` and all GENE-3D field/moment data) and `BPReader` (ADIOS2, optional). `MultiSegmentReader` stitches segments transparently. Key interface: `read_all_times()`, `stream_selected(indices)`, plus `var_names` / `index_of(name)` for reading a variable by name. Canonical variable orders live in `FIELD_VARS` / `MOM_VARS` / `MOM_VARS_3D` / `canonical_vars()`.
- **`geometry.py`** — `Geometry()` function: loads local (spectral), global (real-space) or GENE-3D 3-D geometry, from ASCII or HDF5. Returns dict with metric, curvature, Jacobian, `area`, and for GENE-3D also `profiles` (q, dVdx, sqrtgxx_fs) and `cart_coords`.
- **`coordinates.py`** — `Coordinates()` function: builds kx, ky, z, vp, mu arrays from params. GENE-3D takes `load_coord_xy_global()`, which prefers the grids in `coord<ext>.h5` and falls back to reconstructing them.
- **`utils.py`** — `set_runs()`: discovers output segment suffixes by scanning for `nrg*` files.
- **`profiles_loader.py`** — `load_equilibrium_profiles()`: loads per-species equilibrium profiles from `profiles_<species><ext>` (ASCII, columns `x/a, x/rho_ref, T, n, omt, omn` — in that order) or its `.h5` twin. GENE-3D appends a block per profile update; the last block wins.
- **`omega.py`** — `read_omega()` / `read_eigenvalues()`: parse GENE linear `omega<ext>` / `eigenvalues.dat` files (used as an optional cross-check by the growth-rate diagnostic).

### `diagnostics/` — Physics computations and plotting

**One class per diagnostic, handling every geometry internally.** There is no
per-geometry subpackage: a diagnostic branches on `run.geometry_kind`
(`flux_tube` / `x_global` / `y_global` / `xy_global`) inside its own module, so
there is one place to look for "how is this computed" and one entry point on
`Run`. Diagnostics with no meaning outside GENE-3D set `supported =
("xy_global",)` and the base class refuses on construction.

Shared machinery lives in two private modules — put anything reusable there
rather than re-implementing it in a diagnostic:

- **`_base.py`** — `CachingDiagnostic`: HDF5 persistence (`_load_saved_times`,
  `_select_window`), `_time_average`, and `_sync_field_mom_indices`, which pairs
  field with moment snapshots **by time value**. Use that helper; `istep_field`
  and `istep_mom` need not agree and positional pairing silently correlates a
  field snapshot with moments from another time.
  `_window`/`_bounds` treat a **negative bound as unbounded**, so `t=(500, -1)` is "from 500 to the
  end"; GENE times start at zero, so a negative bound could never select anything.
  `RunDiagnostic`: the Run-native surface every diagnostic inherits — `run`,
  `params`/`coord`/`geom`/`geometry_kind`/`is_3d` shortcuts, `_window`/`_bounds`/
  `_key`/`_indices`, `_common_indices`, `_sources` (which reader holds a
  named variable), `_require`, `_t_average`, and
  `.data`/`.save()`. **When a diagnostic reads more than one file per time step
  (all species' moments, say), use `_common_indices`, not `_indices` per file.**
  GENE-3D writes `mom_<species>` one species at a time, so a running or
  interrupted job leaves one file a snapshot short; streaming each over its own
  indices yields ragged arrays that only fail later at `np.stack`. Set
  `cache_file` on a subclass to make it HDF5-backed.
  **Always time-average through `_t_average` (xarray DataArray) or
  `_time_average` (ndarray), never `.mean("time")`**: GENE's dt is adaptive and
  output happens every `istep_*` *steps*, so output times are unevenly spaced and
  a plain mean is biased by tens of percent (~18% on a realistic axis).
  `test_no_diagnostic_uses_a_plain_time_mean` greps `diagnostics/` to keep it out.
- **`_gene3d.py`** — GENE-3D-specific physics only: `flux_geomfac`,
  `exb_velocity_ky`, `flutter_velocity_ky`, `check_flux_consistency`,
  `to_ky`/`to_kx`, `flux_surface_average`, `xz_average`, `jacobian_yz`,
  `index_window` (the one coordinate-window helper; `radial_slice` wraps it and
  adds the `buffer_frac` trim), `z_average_ky` / `radial_weights` (the pair that
  lets a diagnostic keep the radial axis and reduce later without changing the
  answer — `xz_average` collapses both at once).

Diagnostics available for every geometry:

- **`nrg.py`** — `NrgReader`: energy/flux time traces (reads `nrg` directly)
- **`spectra.py`** — `Spectra`: flux spectra, **all three geometries in one class** (this absorbed the former `spectra_global.py`). Flux tube = kx/ky/z spectra with Hermitian ky weighting. **Both global geometries produce the same three products** — `*_xky` (the `(x, ky)` map, the primary quantity), `*_x` (flux profile summed over ky) and `*_ky` (spectrum averaged over the retained radial window) — so a script needn't know which global geometry it has. **No radial trimming or averaging by default** — `buffer_frac` is 0, so the whole domain is kept, and `plot()` leads with the 2-D `(x, ky)` map (`which=` selects among `map`, `ky`, `profile`, `all`; a flux tube refuses `which` rather than ignoring it, since x is spectral there). GENE-3D rebuilds the maps from φ and the moments and cross-checks them against the fluxes the code wrote itself (`.consistency`); keeping the x axis preserves that check exactly, because a joint x-z average equals the per-x z-average followed by an x-average weighted by `_gene3d.radial_weights` (`sum_z J`). **Flux names follow the package convention everywhere** — `Gamma_es`/`Q_es`/`Pi_es` + `_em` — where the flux tube used `G_es` and x-global used `Qes`/`Ges`/`Pes`; `_current_name` translates legacy HDF5 keys on read and `_migrate_cache` renames them in place **before** the "is anything missing?" early return, since an unmigrated append raises `KeyError` after already extending `time`. Two cache files (`_CACHE_FILES`, picked from `geometry_kind`): the schemas differ in dimension order and grouping. **Momentum flux (`Pi_*`) exists only for x-global** — neither the flux tube nor GENE-3D computes it
- **`contours.py`** — `Contours`: **every** projection of a snapshot — the three planes (`xy`, `xz`, `yz`) and the three 1-D lines (`x`, `y`, `z`), picked with `reductions=` (default `("xy", "xz")`, `"all"` for the lot). This absorbed the former `slices.py`. **One interface for every geometry**, and `compute`/`dataset`/`plot` all work on all of them — the spectral paths used to take a different option set (`field=` index, `ifft=`, `iz`/`iy`, `show_xz`) and had no `.dataset()`; those names are listed in `_RETIRED` and raise a `TypeError` naming their replacement. `x_fourier`/`y_fourier` name **the view, not the transform**: `False` means "show real x/y" and the class inverse-transforms a flux tube to honour it exactly as `True` forward-transforms GENE-3D, so **real space is the default everywhere**. **The y inverse is only valid once x is real** — GENE stores `ky >= 0` because the *real-space* field is real, giving `f(-kx,-ky) = conj(f(kx,ky))`, so at fixed kx the ky axis is not Hermitian; a flux tube is inverted in x first, and a kx-with-real-y view goes back out to kx afterwards. A view that matches what the run already stores costs no round trip. The magnitude is taken **once at the end**, so `x_fourier` and `y_fourier` together mean `|FFT_xy(f)|` — GENE-3D previously computed `FFT_y(|FFT_x(f)|)`. The real-space y axis has **`2*nky0`** points (`discretization.F90:479`, `ly0da = 2*nky0`); the old spectral path used `2*(nky-1)` and was two short. A reconstructed real axis is 0-based (the grid the transform actually produces, and GENE-3D's own convention); genuinely radial x stays `x/a`, and `ds.attrs["x_label"]` says which. **Every dropped coordinate is cut at the grid point nearest zero, not averaged** — so `xy` is the plane at `z=0` and `xz` at `y=0` — because a bare z-average smears the outboard and inboard sides of a global run together; pass `ylim`/`zlim` to average over a range instead, in the units of the axis you are looking at. **`x` is the exception**: it defaults to the whole range, since a global run's radial grid never crosses zero. Options are per-call and never sticky: selection happens in `compute`/`dataset`/`plot`, not in `__init__`
- **`shearingrate.py`** — `ShearingRate`: zonal potential, E_r, v_ExB, ω_ExB. This absorbed the former `zonal.py`. **Every geometry returns the same variable names** — `phi_zonal`, `e_r`, `v_exb`, `omega_exb`, `shearing_rms` — which the two paths did *not* agree on before (`phi_zonal_x`/`omega_ExB` on spectral runs vs `phi_zonal`/`omega_exb` on GENE-3D), so there is now one plot implementation for all three geometries. `load()` translates the old HDF5 key names and `_migrate_legacy_names` renames them in place before appending, so an existing `shearing_rate.h5` keeps working — without the migration an append to a legacy cache raises `KeyError` **after** already growing `time`. `plot(which=...)` picks figure groups: `maps`, `profiles`, `summary`, or `zonal` for the φ_zonal x-t map beside its time-averaged profile (the view `Zonal` used to give — the spectral paths cached `phi_zonal` but never plotted it). **One ExB convention for every geometry**, fixed by GENE itself, not chosen here: `e_r = -dphi/dx`, `v_exb = -e_r/C_xy`, `omega_exb = d(v_exb)/dx` — matching `profiles.F90`'s `ExBrate = -dEraddx_prof(0)`. The flux tube was already right; **x_global had `v_exb = e_r`** (opposite sign, no `C_xy`) and **GENE-3D had `+e_r/C_xy`** (opposite sign), and `gam.py` matched GENE-3D. The full derivation is in the `_gene3d.py` convention note. x_global's `omega_exb` deliberately keeps the Lapillonne Eq. 5.20 form with the q-profile, so there it is **not** `d(v_exb)/dx`. Also: for x_global the Jacobian is `(nx, nz)` and must be normalised **per flux surface** (`axis=-1`) — `J/J.sum()` over the whole array made every flux-surface average ~`1/nx` too small, x-dependently. GENE-3D uses `C_xy` only — the `1/sqrt(g^xx)` of `flux_geomfac` belongs to a flux, not a flow
- **`profiles.py`** — `Profiles`: flux-surface-averaged radial profiles; GENE-3D builds the total profile as `temp*(temp_prof + rhostar*minor_r*<T>_FS)` — in **T_ref** units, matching GENE-3D's own `profile_<species>`, which writes `spec%temp*(...)`. Two factors to get right: omitting `rhostar*minor_r` overstates the perturbation by `1/rhostar`, and normalising to the *species* temperature instead of `T_ref` leaves the dataset and its SI conversion a factor `temp` off. `compare_with_code()` checks both — but only on a run where some species has `temp != 1`. **`plot()` draws one figure per species**, each panel showing the time-averaged total (solid) with its equilibrium background (dashed) — species get a figure each because the species factors make their profiles differ in magnitude, so sharing an axis compresses the smaller one into the baseline. `maps=True` adds the `(t, x)` evolution heatmaps, one extra figure per species, on **both** paths (off by default). **`t` is the averaging window only** — the map always spans every output time in every segment, with the window drawn on it as dashed lines, because its job is to show where in the run the average came from. So `plot()` computes over the whole run regardless of `t`. An empty window raises rather than averaging to NaN. `si=True` converts **every** panel that has a conversion — `_SI_SPEC` covers T, n, u and the three gradients (`Lref**-1`, giving m^-1); a quantity absent from it stays normalised and is labelled as such, never a normalised number under an SI heading. Two unit vocabularies are kept apart on purpose: `unit_attr` gives the plain-text dataset `units` attribute (`T_ref`, `L_ref/L_T`) and `unit_label` the LaTeX axis label. Note `si` used to be dropped entirely on the regular-GENE path — `plot()` never forwarded it to `_plot_spectral`, so `plot(si=True)` silently redrew the same figures
- **`fluxes2d.py`** — `Fluxes2D`: x-resolved fluxes. Spectral paths reconstruct from φ; GENE-3D reads its own `Gamma_*`/`Q_*` from the moment file, and the integration area follows `norm_flux_projection`. `plot()` draws three figures — gyro-Bohm profiles, SI profiles, and the `(x, t)` map. Each profile panel shows the total (solid) with its ES (dashed) and EM (dotted) parts, colour per species; `components=False` for totals alone. `volume_average(t=None, prefactor=True)` reduces the radial profile to the single number `nrg` reports — `sum_xyz f J / sum_xyz J`, built as the existing per-surface `(y,z)` average followed by an x-average weighted by `sum_{y,z} J` (`_gene3d.volume_average`), which is exact rather than approximate. **Neither a plain x-mean nor a `dVdx`-weighted one is this**: `dVdx` carries a radially varying `C_y`. The species factor is **always** applied — `dens` to the particle fluxes, `dens*temp` to the heat fluxes, ES and EM alike — because that is what `nrg` does (`diag_3d.F90:698-701`, after `sum_3d_real`) and the mom file carries none of its own (written at line 547, before those lines). There is deliberately no way to switch it off. The reference GUI (`diag_fluxesgene3d.py:88-97`) scales only the ES terms, which leaves its EM fluxes short by `dens`/`dens*temp` and inconsistent with its own ES terms. **Time-average with `_t_average` (trapezoidal), never `.mean("time")`**: GENE's dt is adaptive and output is every `istep_mom` *steps*, so output times are unevenly spaced and a plain mean is biased by tens of percent
- **`amplitude.py`** — `AmplitudeSpectra`: `|f|²` spectra. Flux tube reuses `Spectra.averages`; x-global builds `(x, ky)` maps; GENE-3D transforms **both** directions
- **`growthrate.py`** — `GrowthRate`: per-ky γ/ω from the complex amplitude (spectral, optional `omega<ext>` cross-check) or rescaling-aware γ from `max|φ|` (GENE-3D)
- **`profile_diag.py`** — `ProfileDiag`: `profile_<species>`. **13 columns for GENE, 8 for GENE-3D** — the column set is chosen from the geometry; plus `flux_profiles()`
- **`ballooning.py`** — `Ballooning`: field-line mode structure (needs a single ky, so it refuses for GENE-3D)
- **`geometry_plots.py`** — `GeometryPlots`: geometry coefficients, **all geometries**. Dispatches on the *rank* of the arrays, not the geometry kind — the readers use the same keys and only the rank differs (`(nz,)` flux tube, `(nx,nz)` x-global, `(nx,ny,nz)` GENE-3D), so `self.ndim` comes from the Jacobian's shape. It used to require `ndim == 3` and refuse otherwise. `which=` picks views: `overview` (every coefficient in one figure — **the default**, where it used to open one figure per coefficient), `detail` (the per-coefficient cuts/planes, skipped when only a z axis exists), `surface` (Z vs R from the `shape` arrays), `profiles`. **The content is complementary between codes**: a flux tube writes `shape` (gR/gZ/gPhi) and `curv['sloc']`, both of which were previously invisible — `_LABELS` had no `sloc` entry and the shape arrays were never read — while GENE-3D writes `profiles` and an array `area` that a flux tube does not
- **`velocity.py`** — `SrcMom`: Krook source moments as radial profiles. **Both global geometries**, not GENE-3D-only: GENE writes them for a global run too. It refuses for a flux tube because *GENE* refuses — `diag_df.F90` forces `istep_srcmom = 0` when `xy_local` ("not possible in local simulations"), and also for a linear ky>0 run, so the file cannot exist. GENE writes **nine** datasets (`src_label` x `mom_label`, including `f0_term`) where GENE-3D writes six; `H5Reader` discovers them from the file so no branch is needed. Only the HDF5 form is readable — the `write_std` unformatted layout (a time record then one `(nx0, 3)` record per source term) is not what `BinaryReader` decodes, so such a run is reported, not misparsed
- **`velocity.py`** — `VspSlice`: velocity space on the `(z, v_par, mu)` grid, **every geometry**. `diag_vsp.F90` puts no geometry restriction on it, and both codes write the same five quantities as the same `(nz0, nv0, nw0, n_spec)` array — only GENE's `Q_es`/`Q_em` are `Q_ese`/`Q_eme` in GENE-3D, which `H5Reader` discovers from the file, so nothing branches. HDF5 only: GENE's `write_std` form is one unformatted record per snapshot holding the whole 5-D array, not what `BinaryReader` decodes, so such a run is reported rather than misparsed

GENE-3D-only for now (each declares `supported = ("xy_global",)`):

- **`timetraces.py`** — `TimeTraces`: volume-averaged and ky-resolved traces
- **`frequency.py`** — `Frequency`: the **nonlinear** frequency spectrum, `P(ky, omega) = <|phi(ky,omega)|^2>`, after `gene/diagnostics/prog/frequency.pro`'s FFT mode. `GrowthRate`/`Omega` fit one coherent mode per ky and mean nothing once a run saturates; this reports where the band sits (dominant + subdominant peak, spectral centroid) and how wide it is. Welch: Hann window of `window_frac` of the record (0.25, IDL's `twin`), 50% overlap, periodograms averaged — a single FFT over the whole record is one realisation of a random process and far too noisy to read a peak off. **The sign convention is load-bearing**: GENE's ansatz `exp(i(k·x - omega t))` means the `+ky` component of a `+y`-travelling wave varies as `exp(-i omega t)`, and numpy's forward FFT carries the same kernel, so the frequency axis is the **negated** `fftfreq`. Getting it backwards mirrors the spectrum and reverses every propagation direction while looking perfectly healthy — `test_travelling_wave_lands_at_positive_omega` pins it against a wave built from the ansatz by hand. Two departures from IDL: each segment's **mean is removed** (a finite time-average is not a fluctuation, and it drags the centroid to zero; `detrend=False` restores IDL's behaviour), and **`omega_nyquist = pi/dt` is reported and checked** — `dt` is the *output* cadence, `istep_field` steps of an adaptive dt, and a nonlinear run written too sparsely aliases its real frequencies back into range, which nothing downstream can detect. **Time cannot be streamed away here** — the whole series is needed before the transform — so **z is cut at the grid point nearest zero, not averaged** (`zlim` widens it deliberately, at `nz`× the memory), and `compute()` reports the array size. Reductions happen **after** the transform, Jacobian-weighted: averaging complex amplitudes first cancels the phase the frequency lives in
- **`gam.py`** — `Gam`: zonal-flow oscillation; refuses when there is no zonal component above round-off rather than fitting a frequency to noise
- **`chi.py`** — `ChiGradient`: χ vs. the self-consistent driving gradient; reuses `Fluxes2D` and `Profiles` so all three agree on normalisation
- **`omega.py`** — `Omega`: frequency view over `GrowthRate`
- **`planes.py`** — `Planes`: remap onto geometric (θ, φ) angles + (n, m) analysis
- **`vis.py`** — `Vis`: VTK export using the `cart_coords` GENE-3D writes

### Data flow
1. `set_runs(folder)` → segment suffixes
2. `Params(folder, extensions)` → parameter dicts
3. `Geometry(...)` + `Coordinates(...)` → geometry/coordinate arrays
4. `BinaryReader`/`BPReader`/`MultiSegmentReader` → stream field/moment data
5. Diagnostics consume readers → compute → cache to HDF5 → plot

### `__init__.py` flat re-exports
The root `__init__.py` re-exports all major classes for backward compatibility (`from genetools import Params, BinaryReader, ...`).

## Key conventions

- **Geometry kinds**: prefer `params.geometry_kind()` / `run.geometry_kind` over the two-valued `x_local`. `flux_tube` is spectral in x and y; `x_global` real-space in x; `xy_global` (GENE-3D) real-space in both. `x_local` alone cannot distinguish `x_global` from GENE-3D.
- **Hermitian symmetry**: in the spectral-y cases only non-negative ky modes are stored, so ky=0 is unweighted and ky>0 is weighted by 2 in summations. **This does not apply to GENE-3D**: y is real space, so its FFT already contains both signs of ky and no factor of 2 belongs anywhere.
- **GENE-3D axis order and dtype**: futils stores a Fortran `(nx, ny, nz)` array as an HDF5 `(nz, ny, nx)` dataset, and `creatf(..., 's')` stores 32-bit reals whatever `PRECISION` says. `H5Reader` undoes both; never derive the dtype from the namelist.
- **GENE-3D flux conventions** (from `diag_3d.F90` / `geometry.F90`, not guesswork): `Gamma_es = -n·∂φ/∂y·flux_geomfac` and `Gamma_em = +u_par·∂A∥/∂y·flux_geomfac`, where `flux_geomfac = 1/C_xy`, times `1/sqrt(g^xx)` when `norm_flux_projection`. There is **no** `1/Bref`, and the flutter velocity has the **opposite sign** to the ExB one. The same flag decides which area converts a flux density to a total (`area['Area']` when projecting, `area['dVdx']` otherwise).
- **`n_moms` is unreliable for GENE-3D**: the parameters file reports `par_in::n_moms = 6` while `diag_3d::n_moms = 10` datasets are written. Discover variables from the file (`reader.var_names`).
- **`nrg` column order** (`diag.F90:nrg_label1`): `n², u_par², T_par, T_per, Γ_es, Γ_em, Q_es, Q_em, Π_es, Π_em, ...`. GENE-3D writes the first eight (`nrgcols = 8`).
- **numpy compat**: code handles both `np.trapz` (numpy <2.0) and `np.trapezoid` (numpy >=2.0).
- **Optional deps**: `adios2` for BP files, `numba` for JIT acceleration in spectra. Both use try/except import guards.
- **Temperature**: T = (1/3)T_par + (2/3)T_perp throughout diagnostics.
